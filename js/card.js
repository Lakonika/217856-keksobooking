...
(function () {
  ...
  var onKeydownEsc = function (evt) {
    if (evt.key === ESCAPE) {
      var currentCard = window.map.querySelector('.map__card');
  ...
  var getPhotosFragment = function (photos) {
    ...
    photos.forEach(function () {
      ...
      newPhotoImg.src = window.getRandomCollection(window.getRandomNumber(1, window.OFFER_DETAILS.PHOTOS.length - 1), window.OFFER_DETAILS.PHOTOS);
  ...
  window.renderCard = function (offerObject) {
    var newOfferCard = window.template.querySelector('.map__card').cloneNode(true);
    ...
    newOfferCard.querySelector('.popup__type').textContent = window.OFFER_DETAILS.houseTypesTranslation[offerObject.offer.type];
    ...
    if (currentCard) {
      return window.map.replaceChild(newOfferCard, currentCard);
    }
    window.map.appendChild(newOfferCard);
    return newOfferCard;
  };
